...
- ~~**A real monster sprite.**~~ **Done, and now it is PAINTED.** It was
  a coloured box, then two hand-written rectangle lists, and it is
  `assets/sprites.png` — an indexed PNG against `assets/amaze-mode0.gpl`
  — compiled by `genspr.py` into the same `db col, ncol, band0, band1,
  pen` records. The budget worry below was the right one and it is what
  the compiler optimises: `sprcover.py` minimises **microseconds**
  ...
  generated by hand whenever someone remembered, so a change to the
  monster changed nothing on the disc. It is in `GEN` now and
  `assets/sprites.png` is in `ART`.
