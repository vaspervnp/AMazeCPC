...
| `emu_pace.py 600` / `emu_pace3.py` | **LOCKED: True**, 4800 and 10144 game frames, 100% at 10 vsyncs. The old MIXED was a SPREAD tolerance measuring the sampler: 199.68 ms is 798.72 samples of 250 µs, so intervals must land on 798 or 799 whatever the game does. Over eight periods the **sum** is within 0.31 ms of 8×199.68 — the jitter is non-cumulative, it is where inside the frame `frame_ctr` is bumped — so the cadence is tested on the **mean** now |
